import { ADD_TO_CART, INIT_GLOBAL_STATE, REMOVE_PRODUCT, SET_TAB_STATUS, SET_SECTION_STATUS } from './actionTypes';
...
const reducer = (state = initialState, action) => {
	switch (action.type) {
		...
		case REMOVE_PRODUCT:
			const filtered = state.products.filter(prod => prod.id !== action.payload);
			return {
				...state,
				products: filtered,
			};
